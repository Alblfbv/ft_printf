...
/*   Updated: 2019/01/16 14:03:04 by allefebv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_init_table(t_fptr_preci *table)
{
	...
	table->preci_group[0] = strdup("s");
	table->preci_group[1] = strdup("diouxX");
...
char		*ft_process_preci(t_conv_spec conv_spec, char *str)
{
	...
	if (conv_spec.precision != -1)
	{
		...
		{
			if (strchr(table->preci_group[i], conv_spec.conv_id))

...
char	*ft_process_X(t_conv_spec conv_spec, va_list *ap)
{
	...
	if (conv_spec.modifier[1] == 1)
		return (ft_strupcase(ft_itoa_base((unsigned short)va_arg(*ap, unsigned int), 16)));
	if (conv_spec.modifier[2] == 1)
		return (ft_strupcase(ft_itoa_base_ll(va_arg(*ap, unsigned long), 16)));
	if (conv_spec.modifier[3] == 1)
		return (ft_strupcase(ft_itoa_base_ll(va_arg(*ap, unsigned long long), 16)));
